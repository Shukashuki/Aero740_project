...
- **02b — Reward Component Ratio：** qpos 佔 cost 超過 99.7%，qvel 幾乎可忽略。
...
- **03b — Tick Boundary Discontinuity：** tick 邊界 ΔQpos 絕對值 heatmap — mean 0.004, max 0.068。大部分 DOF 的 tick boundary 非常平滑（warm start 有效）。Max 出現在 finger joints，可能是 contact transition 造成的 replanning 跳變。
...
- **04b — Ctrl Discontinuity Heatmap：** tick 邊界 ΔCtrl 絕對值 — mean 0.012, max 0.089。比 qpos 跳變更明顯但仍可接受。
...
- **06a — Reward Across Rollouts：** p36-tea 的 rollout 0 在所有 17 個 tick 都是 outlier（reward 顯著低於 mean − 1σ）；p44-dog / p52-instrument 一致性高。
...
2. **不同 DOF 的行為差異是合理的：** 同時優化 ≠ 同等重要。Wrist 負責大範圍移動，fingers 負責精細接觸。qpos 佔超過 99.7% 的 cost share，說明策略是 **「先到位，再談力」**。
